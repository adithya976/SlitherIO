...
class Segment:
    ...
    def draw(self, surface, trailing):
        self.rect = pygame.draw.circle(surface, (0, 0, 255) if trailing else (0, 255, 255),
                                       self.game.camera.transformed_coords(self.pos), 15)
    ...
    def update(self):
        ...
        if mouse_pos != v2(0, 0):
            self.direction = self.direction.rotate(
                motion_angle / 100 if abs(motion_angle) < 180 else -motion_angle / 100).normalize()
        else:
            self.direction = v2(0, 0)

        self.segments = self.segments[:-1]  # Remove last segment
        self.segments.insert(0, Segment(self.segments[0].pos + self.direction * 1,
                                        self.game))  # Insert new segment at the front
...
class Score():
    ...
    def update(self):
        self.text = [self.font.render(f"Score:", True, self.color),
                     self.font.render(f"{self.game.player.uid}:{self.game.player.score}", True, self.playerColor)]
        for opp in self.game.opponents:
